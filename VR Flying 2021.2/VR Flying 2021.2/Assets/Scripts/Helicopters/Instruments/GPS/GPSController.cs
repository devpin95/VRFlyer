...
public class GPSController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (timeCounter < gpsInfo.updateTime) return;
        ...
        Vector3 localMapPosition = new Vector3(-1 * mapPosition.x + (containerSizeDelta.x / 2), -1 * mapPosition.y + (containerSizeDelta.y / 2), 0);
    ...
    public void SetGPSTarget(GPSGUI.GPSDestination des)
    {
        ...
        timeCounter = gpsInfo.updateTime;
    }
    ...
    private void UpdatePathLines()
    {
        if (update)
        {
            targetPathLine.point1 = targetLocation;
            targetPathLine.point2 = mapPosition;
    ...
    private void UpdateETAText()
    {
        ...
        if ( _targetDestinationSet )
        {
            ...
            float metersPerSecond = instrumentInputs.VelocityMagnitude;
            
            if ( metersPerSecond > 0 )
            {
                // if velocity is not 0, convert meters per second to miles per second
                float milesPerSecond = metersPerSecond / Utilities.MetersPerMile;
                
                // get the distance in meters to the destination
                float metersToDestination = Vector2.Distance(_targetDestination.worldPos.Flatten(), worldPosition.Flatten());
                
                // convert the distance in meters to distance in miles
                float milesToDestination = Utilities.UnityDistanceToMiles(metersToDestination);

                // get the time in seconds, then minutes, to the destination at the current velocity
                float timeInSeconds = milesToDestination / milesPerSecond;
                float timeInMinutes = timeInSeconds / 60;

                // set the string to the time in minutes to the destination
                etaString = "~" + timeInMinutes.ToString("n2");
            }

            destinationETA.text = etaString;
        }
    }
